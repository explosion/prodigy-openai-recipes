...
in a quantifiable and blind way. You can pass multiple prompts, as well as a set of data that will fill
both.
...
| `prompt1_template_path`| Path  | The first jinja2 prompt template                     |                      |
| `prompt2_template_path`| Path  | Path to second jinja2 prompt template                |                      |
| `--model`, `-m`        | str   | GPT-3 model to use for completion                    | `"text-davinci-003"` |
...
Let's try to generate humerous haikus. To do that we first need to construct two jinja files that
represent the prompt to send to OpenAI. 
...
When you look at this recipe you'll notice that the title template is rendered and that
