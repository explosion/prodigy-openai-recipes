...
class PromptExample(abc.ABC):
    ...
    # Pyright couldn't understand the types with
    # abc.abstractclassmethod. Not sure whether I was doing it
    # wrong?
    @classmethod
    def from_prodigy(
        cls: Type[_PromptT], example: Dict, labels: Iterable[str]
    ) -> _PromptT:
...
class OpenAISuggester:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.segment:
            # Always sad to type: ignore, but the types for
            # this in Prodigy will be improved soon.
    ...
    def set_hashes(self, stream: Iterable[_ItemT]) -> Iterable[_ItemT]:
        for example in stream:
            # TODO: We need to either keep this abstract
            # (i.e. force users to subclass it)
            # or we can pass in the hash keys to the class.
